            ...
            <div class="card-header py-3">
                <div class="row align-items-center">
                    ...
                    <div class="col-md-6 text-end">
                        <a href="{{ route('admin.product-verification.create') }}" class="btn btn-primary btn-create">
    ...
    <script>
        function deleteProduct(id) {
            if (confirm('Are you sure you want to delete this product? This will also delete all associated images and batches.')) {
                $.ajax({
                    url: "{{ url('/') }}/admin/product-verification/" + id,
                    type: 'DELETE',
                    data: {
                        "_token": "{{ csrf_token() }}"
                    },
        ...
        $(document).on('change', '.change-status-input', function() {
            ...
            $.ajax({
                url: "{{ url('/') }}/admin/product-verification/" + productId + "/change-status",
                type: 'POST',
                data: {
                    "_token": "{{ csrf_token() }}",
                    "status": status
