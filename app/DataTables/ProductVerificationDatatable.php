...
class ProductVerificationDatatable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function (Product $product_verification) {
                return '
                    <a href="' . route('admin.product-verification.edit', $product_verification) . '"
                       class="btn btn-sm btn-primary"
                       title="Edit">
                        <i class="bx bx-edit-alt"></i>
                    </a>
                    <button class="btn btn-sm btn-danger delete-product"
                            onclick="deleteProduct(' . $product_verification->id . ')"
                            data-id="' . $product_verification->id . '"
                            title="Delete">
    ...
    {
        return $product->newQuery()->where('product_type', '1');
    }
            ...
            ->buttons([
                ...
                [
                    ...
                    'action' => 'function(e, dt, node, config) {
                        window.location.href = "' . route('admin.product-verification.create') . '";
                    }'
                ],
                Button::make('excel'),
                Button::make('csv'),
